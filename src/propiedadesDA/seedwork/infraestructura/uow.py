        ...
        for batch in batches:
            for arg in batch.args:
                if isinstance(arg, AgregacionRaiz):
                    return arg.eventos_compensacion
        return eventos
...
def registrar_unidad_de_trabajo(serialized_obj):
    from propiedadesDA.conf.uow import UnidadTrabajoSQLAlchemy
    from flask import session
...
def flask_uow():
    from flask import session
    from propiedadesDA.conf.uow import UnidadTrabajoSQLAlchemy, UnidadTrabajoPulsar
    if session.get('uow'):
        return session['uow']
